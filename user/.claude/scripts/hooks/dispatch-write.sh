...
# Called from ~/.claude/settings.json with: bash ~/.claude/scripts/hooks/dispatch-write.sh "$filePath"
set -o pipefail
...
if $HAS_PHP && [[ "$EXT" == "php" ]]; then
    bash "$HOOKS_DIR/php/validate-php-syntax.sh" "$FILE"
    ...
    bash "$HOOKS_DIR/php/auto-run-new-test.sh" "$FILE"
fi
